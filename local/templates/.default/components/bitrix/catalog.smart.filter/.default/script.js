...
JCSmartFilter.prototype.toggleWait = function(elem, flag)
{
	var form = BX.findParent(elem, {'tag':'form'}),
		wrapper = BX.findParent(form, {'tag':'div'});

	if (!!BX(wrapper)) {
		if (flag) {
			BX.showWait(wrapper);
		} else {
			BX.closeWait(wrapper);
		}
	}
}
...
JCSmartFilter.prototype.postHandler = function (result, fromCache)
{
	...
	if (!!result && !!result.ITEMS)
	{
		...
		if (!!modef_del)
		{
			...
			if (modef_del.style.opacity === '0')
			{
				modef_del.style.opacity = '1';
			}
		}
	}
	...
	this.toggleWait(modef_del, false);
};
