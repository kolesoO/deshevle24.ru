...
foreach ($arResult["QUESTIONS"] as $FIELD_SID => &$arQuestion) {
    ...
    if ($arQuestion["STRUCTURE"][0]["FIELD_TYPE"] != "hidden") {
        ...
        if ($arQuestion["STRUCTURE"][0]["FIELD_TYPE"] == "file") {
            ...
            $class = "col-lg-24 col-md-24 col-xs-24"; //class

            if ($arQuestion["STRUCTURE"][0]["FIELD_TYPE"] == "textarea") {
                $arQuestion["HTML_CODE"] = '<textarea id="'.$id.'" type="'.$type.'" name="'.$name.'" class="'.$class.'" '.$attrs.' placeholder="'.$arQuestion["CAPTION"].'">'.$arQuestion["VALUE"].'</textarea>';
            } else {
                $arQuestion["HTML_CODE"] = '<input id="'.$id.'" type="'.$type.'" name="'.$name.'" value="'.$arQuestion["VALUE"].'" class="'.$class.'" '.$attrs.' placeholder="'.$arQuestion["CAPTION"].'">';
            }
