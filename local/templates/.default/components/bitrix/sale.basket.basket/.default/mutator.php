...
foreach ($this->basketItems as $row)
{
    ...
    {
        $rowData['IMAGE_URL'] = SITE_TEMPLATE_PATH . '/images/no-image.png';
    }
...
$totalData = array(
    ...
    'PATH_TO_ORDER' => $this->arParams['PATH_TO_ORDER'],
    'TOTAL_ITEMS_PHRASE' => Loc::getMessage(
        'SBB_TOTAL_ITEMS_PHRASE',
        [
            '#COUNT#' => $result['ITEMS_COUNT'],
            '#SUFFIX#' => GetDeclNum($result['ITEMS_COUNT'])
        ]
    )
);
...
if ($this->hideCoupon !== 'Y' && !empty($result['COUPON_LIST']))
{
	...
	foreach ($totalData['COUPON_LIST'] as &$coupon)
	{
		...
		{
			$coupon['CLASS'] = 'text-danger';
		}
		elseif ($coupon['JS_STATUS'] === 'APPLYED')
		{
			$coupon['CLASS'] = 'text-success';
		}
		else
		{
			$coupon['CLASS'] = 'text-danger';
