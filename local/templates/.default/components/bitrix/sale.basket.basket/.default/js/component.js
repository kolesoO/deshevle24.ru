;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		{
			var overlay = this.getCacheNode(this.ids.itemListTable);

			if (BX.type.isDomNode(overlay))
			{
				BX.addClass(overlay, "loading");
			}
		},
		...
		hideItemsOverlay: function()
		{
			...
			if (BX.type.isDomNode(overlay))
			{
				BX.removeClass(overlay, "loading");
			}
		},
		...
		sendRequest: function(action, data)
		{
			...
			BX.ajax({
				...
				onsuccess: BX.delegate(function(result) {
					...
					if (result.BASKET_DATA.BASKET_ITEMS_COUNT == 0 && !!this.params.EMPTY_REDIRECT_PATH) {
						location.href = this.params.EMPTY_REDIRECT_PATH;
					}
