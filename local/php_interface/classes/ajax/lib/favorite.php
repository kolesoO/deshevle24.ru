...
class Favorite
{
    ...
    public static function add(array $arParams)
    {
        ...
        if ($arParams["offer_id"] > 0 && !self::isAdded($arParams["offer_id"])) {
            $_SESSION[self::$sessionKey][] = $arParams["offer_id"];
    ...
    public static function delete(array $arParams)
    {
        ...
        if ($arParams["offer_id"] > 0 && self::isAdded($arParams["offer_id"])) {
            foreach ($_SESSION[self::$sessionKey] as $key => $id) {
                if ($id == $arParams["offer_id"]) {
                    unset($_SESSION[self::$sessionKey][$key]);
                    $arReturn["deleted_item"] = $arParams["offer_id"];
     ...
     * @return array
     */
    public function removeAll(array $arParams)
    {
        $arReturn = [
            'js_callback' => 'removeAllFavoriteCallBack',
            'deleted_list' => $_SESSION[self::$sessionKey]
        ];
    ...
    public static function getList(array $arParams)
    {
        ...
        ob_start();
        if (isset($_SESSION[self::$sessionKey]) && count($_SESSION[self::$sessionKey]) > 0) {
        ...
        return [
            "js_callback" => "getFavoriteListCallBack",
            "html" => $return
        ];
    }
}
